...
# Dataframe of Forward Sim Secondary Infections ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    if (contacts == 0){
      ...
      for (j in 1:contacts){
        ...
        transmission <- 1 #rbinom(1,1,transmissionrate)   #should this be transmission rate or AUC of IIP???
        if (transmission == 0){
...
# Data Wrangling __________________________________________________________________________________________________________________________
  ...
  maxgen = nrow(genbysim) #max(genbysim$generation)
...
#  ________________________________________________________________________________________________________________________________________
  ...
  # queuedata - queue outputs for each person (newpersonid), time at which they arrive (arrivaltime), time selected by the distribution of arrivals (interarrival),
  #             time at which they depart (departuretime), difference btwn arrival and departure (timespent) aka departure selected from distribution
  #             names(queuedata) <- c('newpersonid','arrivaltime','interarrival','departuretime','timespent')
  ##print(queuedata)
  
  # numindiv - ordered increasing timing of an arrival or departure (timing) from proximity denoted by 1 or -1, respectively (arrivORdept) to compute 
  #             cumulative number of individuals within proximity at each timing (sus_count)
  #             names(numindiv) <- c('timing', 'arrivORdept') + sus_count
  ##print(numindiv)
  
  # contactdata - MAIN OUTPUT* ordered by increasing generation interval, each individual with an identifier (personnumber) receives an infection at time (secondinfecttime)
  #               from an individual from the initial cohort infected at time (initcohorttime) and the time between these values (genint)
  #               names(contactdata) <- c('personnumber','initcohorttime','secondinfecttime','genint')
  ##print(contactdata)
  
  # namedintervals - intermediate step to compute all outputs found in genbysim for each simulation iteration. All outputs of this after appended to genbysim
  
  # genbysim - contains all the generation intervals values (generation) with an appended number for cumulative purposes (orderofinf) which are normalized (normcsum) 
  #             per simulations performed (simnumb) from the input parameter 'iterations'
  ...
  p1 <- genbysim %>%
    ...
    geom_histogram(aes(y=after_stat(density), color = simnumb),alpha=0.1, binwidth = 0.025) + 
    geom_density() + 
    ...
    ggtitle(paste0("DeltaPDF FGIs with Beta = ", transmissionrate, " Gamma = ", recoveryrate,".")) + # "initcohort type:", initcohorttype, 
    facet_wrap(~simnumb)
  ...
  p2 <- genbysim %>%
    ...
    ggtitle(paste0("DeltaCDF FGIs with Beta = ", transmissionrate, " Gamma = ", recoveryrate, "initcohort type:", initcohorttype, ".")) +
    facet_wrap(~simnumb)
  
  # eventually make output type as part of function call ?
  print(p1)
  print(p2)
  #}
}
